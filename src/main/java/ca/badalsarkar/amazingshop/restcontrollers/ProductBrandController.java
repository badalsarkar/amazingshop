...
public class ProductBrandController {
    ...
    @GetMapping("/brands")
    public ResponseEntity<CollectionModel<EntityModel<ProductBrand>>> all(){
        List<EntityModel<ProductBrand>> brands=brandRepository.findAll()
                .stream().map(brandAssembler::toModel).collect(Collectors.toList());
        return ResponseEntity.ok().body(CollectionModel.of(brands,linkTo(methodOn(ProductBrandController.class).all()).withSelfRel()));
    }

    @GetMapping("/brands/{id}")
    public ResponseEntity<EntityModel<ProductBrand>> one(@PathVariable Long id){
    ...
    @PostMapping("/brands")
    public ResponseEntity<EntityModel<ProductBrand>> newBrand(@RequestBody ProductBrand newBrand){
